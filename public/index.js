const SERVER_URL = "http://192.168.0.124";
...
const renderPagination = ({pages, itemsPerPage, currentPage}) => {
    ...
    if (offset > length) {
        ...
        for (let item = (length - currentPage) < 4 ? length - 4 : currentPage; item <= end; item++) {
            buttons += `<button class="btn-pagination" value="${item}">${item}</button>`;
        }
    }   
...
const pageCounter = () => {
    if (state.currentPage === 1) {
        document.querySelector('.page-counter').innerHTML = `${state.currentPage} - ${state.itemsPerPage}`;
    } else if (state.currentPage * state.itemsPerPage > state.pages.length) {
        document.querySelector('.page-counter').innerHTML = `${state.currentPage * state.itemsPerPage - state.itemsPerPage} - ${state.pages.length}`;
    } else {
        document.querySelector('.page-counter').innerHTML = 
        `${state.currentPage * state.itemsPerPage + 1} - ${ state.currentPage * state.itemsPerPage + state.itemsPerPage}`;
    }
}
...
const renderFooterSeries = () => {
    ...
    state.initialPages.reduce((state, next) => {
        if (!state.includes(next.dataset.series)) {
            ...
            link.classList.add('d-inline-block', 'm-1', 'px-1', 'link-series', 'text-white');
...
window.addEventListener('DOMContentLoaded', () => {
    // Load initial state
    state.initialPages = Array
    .from(document.querySelectorAll('.videos-item'))
    .sort((a, b) => {
    ...
    document.querySelector('.link-liked').addEventListener('click', (e) => {
        e.preventDefault();
        state.pages = [...state.initialPages];
        state.pages = state.pages.filter(entry => entry.dataset.status === 'true');
        state.currentPage = 1;
        document.querySelector('.pagination-container').innerHTML = renderPagination(state);
        document.querySelector('.btn-pagination').classList.add('active');
        document.querySelector('.browse-counter').innerHTML = `<strong>Browse ${state.pages.length} files</strong>`;
    ...
    document.querySelector('.form-search').addEventListener('submit', (e) => {
        e.preventDefault();
        const inputSearch = document.querySelector('.input-search').value.trim().toUpperCase().split(' ');
